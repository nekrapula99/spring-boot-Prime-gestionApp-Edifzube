...
import com.edifzube.gestionApp.service.IngresosService;
import com.edifzube.gestionApp.service.ItemService;

@Controller
public class IngresosController extends Control implements Serializable{
	...
	private List<Ingresos> filtered;
	...
	public void  getIngresos() {
		ingresosList = ingresosService.getIngresos();
	}
	...
	public void accionNuevo() {
		...
		ingreso.setFecha_creacion(new Date());
	...
	public void modificar() {
		try {
			...
			itemsList = itemService.getItems();
			ingreso.setFecha_actualizacion(new Date());
			codingresoAux = ingreso.getCodingreso();
			detalleAux = ingreso.getDetalle();
	...
	public void save() {
		try {
			...
			System.out.println("ingreso en save: "+ingreso);
			ingresosService.save(ingreso);
			...
			if (isDuplicateException(e)) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,"Error","Ya existe un Ingreso con el codigo: "+ingreso.getCodingreso()));
				return;
			} else {
				e.printStackTrace();
	...
	public List<Ingresos> getFiltered() {
		return filtered;
	}

	public void setFiltered(List<Ingresos> filtered) {
